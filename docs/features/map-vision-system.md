...
3. **Continent Layout**: Worlds inherit a fixed 10×10 continent lattice (`K00`–`K99`) where each continent spans exactly 100×100 fields. The historical "core" refers to the inner `K44`, `K45`, `K54`, `K55` block; everything beyond those coordinates is the expandable "rim". Generation scripts (`generate-regions.ts`, `generate-world.ts`) must never mutate continent boundaries—new spawn space extends by enabling additional K-cells rather than resizing existing ones. Any UX copy that references continents should reuse helper formatters from `lib/map-vision/coordinate-service.ts` so `Kxy` math stays canonical.
4. **Tile Types & Tags**: Empty terrains, oases (with resource bonuses + owner), villages (owner, tribe, population, wall, capital flag, hero presence), specials (artifacts, World Wonders, grey-zone). Terrain cosmetics optional but reserved for future modifiers (§5.3, §7.3).
5. **Distance & Travel**: Euclidean distance in tiles; travel time `distance / speed / world_speed`. Straight-line paths; implement route sampling for contact detection. Terrain penalties optional; stored as modifiers on tile edges.
6. **Edge Behavior**: Default planar map forbids crossing boundaries; UI clamps panning. Toroidal variant wraps coordinates and distance; annotate world rules and adjust search/bookmark resolution accordingly. Provide clarifying note that recon/light trails wrap seamlessly.
7. **Map Scales**: Region (15×15) for tactical operations, Province (100×100) for ops planning/heatmaps, World full extent for strategic overlays. Each scale defines level-of-detail budgets for markers and fog fidelity.

### 3.1 Mini-map & Political Map Expectations
- **Mini-map purpose**: The always-on overview (top-right on desktop, collapsible on tablet) must summarize an area at least 5× the visible main grid so operators can maintain situational awareness without panning. Default render falls back to alliance-color density and barbarians/empty tiles in grayscale.
- **Political map toggle**: Premium accounts unlock a "Political map" switch. When active, the mini-map recolors continents by tribe/ownership overlays, surfaces player markers, and mirrors any color assignments from the marker/highlight system. Implementation-wise, this is a palette swap driven by the same data as the main grid; avoid recomputing visibility.
- **Coordinate readouts**: Hovering or tapping the mini-map should reveal the current center coordinate and K-label (e.g., `475|724 — K55`). The tooltip copy must mention core vs rim when a coordinate resides within `K44`–`K55` or outside.
- **Accessibility**: Provide non-color cues (patterns, badges) for political overlays so players with color-vision deficiencies can distinguish territories. Tie the cues back to the legend described in §9 overlays.

### 3.2 Player Reading Workflow Reference
1. **Locate the tile**: Every UI surface that presents the map (main grid, minimap, political map, tactical popovers) must surface the `x|y` coordinate pair so players can ground their navigation. Tooltips and drawer headers should match the canonical parsing rules in `lib/map-vision/coordinate-service.ts`.
2. **Translate to continent**: When players read coordinates, reinforce the fixed-hundred-band logic that yields the `Kxy` continent label. Display helpers should highlight the matching 100×100 band on both axes, producing the same output as generation scripts. Example copy: `475|724 — K55 (core)`.
3. **Cross-check with the minimap**: Highlight the coordinate on the minimap overview and ensure the same highlight appears on the political map overlay. This lets planners confirm tribe borders, enclave positions, and frontline gaps without dragging the main map.
4. **Scale up when needed**: Premium zoom states (20×20, 30×30, or custom sizes) must keep the hundred-band overlays and highlight colors aligned so users can sketch launch zones or rim expansion without losing their bearings. Contextual helper text should remind players that rim growth occurs by unlocking additional `K` cells, not by resizing existing ones.
